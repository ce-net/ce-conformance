...
`runners/rust/.cargo/config.toml` is generated by `ce-dev-link` (machine-specific paths) and is
git-ignored — never commit it.

## Self-containment status (known gap)

By nature this repo is a *meta*-harness: it drives the four SDK repos, so it cannot be entirely
standalone. The CE standard is "cross-repo deps are github git deps; a fresh clone builds against the
published sources; a local checkout override is a convenience, not a requirement." Status: the Rust
runner now meets it (`Cargo.toml` uses the `git = "https://github.com/ce-net/ce-rs"` dep; a local
`[patch]` config generated by dev tooling opts into sibling WIP). The others do NOT yet: the Go
runner uses `go.mod` `replace ../../../ce-go`, the TS runner imports `../../../ce-ts/dist`, and the
Python runner reads `$CE_PY_DIR`. **TODO** (tracked): switch those to a github dep by default
(`go get …@main`, `@ce-net/sdk` / `github:ce-net/ce-ts`, a vendored `ce.py`) with the local WIP
checkout as an opt-in override — so a fresh clone runs against the published SDKs. Until then, run
the Go/TS/Python runners from a full local workspace checkout (SDK repos present as siblings).
